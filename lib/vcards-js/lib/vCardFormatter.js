...
(function vCardFormatter() {
	...
	module.exports = {
		...
		getFormattedString: function(vCard) {
			...
			if (vCard.url) {
				formattedVCardString += 'URL' + encod
				ingPrefix + ':' + e(vCard.url) + nl();
			}
				...
				for (var key in vCard.socialUrls) {
					if (vCard.socialUrls.hasOwnProperty(key) &&
						vCard.socialUrls[key]) {
							let index = 0;
							formattedVCardString += 'item' + index + '.TYPE:' + key + nl();
							formattedVCardString += 'URL:' + vCard.socialUrls[key].replace("https://","www.") + nl();
							

							// formattedVCardString +=formattedVCardString.replace(`X-SOCIALPROFILE;PREF=${index+1}`,`X-SOCIALPROFILE;${arr[index].uri.replace("https://","www.")}`)+ nl();
